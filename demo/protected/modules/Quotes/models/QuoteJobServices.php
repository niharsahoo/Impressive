...
class QuoteJobServices extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('job_id, quantity', 'numerical', 'integerOnly'=>true),
			array('unit_price_rate, total', 'length', 'max'=>10),
			array('image', 'length', 'max'=>255),
			array('service_description, notes', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, job_id, service_description, quantity, unit_price_rate, total, image, notes, created_at', 'safe', 'on'=>'search'),
		);
	}
