...
class DefaultController extends Controller
{
 ...
 public $layout='//layouts/column1';
	...
	// CgridView Records/page section
		...
		$list_display_for_client_model = new ListDisplayForClient('search');
		$list_display_for_client_model->unsetAttributes();  // clear any default values
		if(isset($_GET['ListDisplayForClient']))
		$list_display_for_client_model->attributes=$_GET['ListDisplayForClient'];
		...
		$this->render('index', array(
			...
			'list_display_for_client_model' => $list_display_for_client_model, //5
	...
	//End Update ListAccessType
	 ...
	 public function actionAdd_list_display_for_client() { 
		...
		$list_item_id = isset($_POST['list_item_id']) ? $_POST['list_item_id'] : '';
		$list_display_for_client_name = isset($_POST['list_display_for_client_name']) ? trim($_POST['list_display_for_client_name']) : '';
		
		if(! empty($list_display_for_client_name)) {
		$model = ListDisplayForClient::model()->findByAttributes(array('name'=>$list_display_for_client_name));
		if(isset($model->id) && $model->id > 0 ) {
			echo '1'; exit;
		}
		else {
			...
			$model->list_item_id = $list_item_id;
			$model->status = '1';
			$model->name = $list_display_for_client_name;
			$model->save();
		}
		}
	} 
	...
	//Update ListDisplayForClient
	 public function actionUpdate_list_display_for_client() { 
		...
		$list_item_id = isset($_POST['list_item_id']) ? $_POST['list_item_id'] : '';
		$list_display_for_client_id = isset($_POST['list_display_for_client_id']) ? trim($_POST['list_display_for_client_id']) : '';
		$list_display_for_client_name = isset($_POST['list_display_for_client_name']) ? trim($_POST['list_display_for_client_name']) : '';
		
		if(! empty($list_display_for_client_name)) {
		
			$model= ListDisplayForClient::model()->findByPk($list_display_for_client_id);
			$model->list_item_id = $list_item_id;
			$model->status = '1';
			$model->name = $list_display_for_client_name;
			$model->save();
